...
- **Zoom iOS su tutti gli input**: aggiunto `@supports (-webkit-touch-callout: none)` in `app.css` che forza `font-size: 16px` su tutti gli input solo su iOS Safari, prevenendo lo zoom automatico al focus
...
- **Font-weight nomi e titolo header a 600**: nomi card e titolo header portati da `500` a `600` per maggiore leggibilità
...
- **Badge urgenza sul tab Spese**: puntino rosso discreto sul tab Spese quando ci sono bollette scadute o in scadenza entro 5 giorni
...
- **Header**: aggiunta icona chiave stilizzata (accent) a sinistra del titolo; titolo su due righe ("Casa Criscuolo" + "/ Falabella" italic accent) coerente con la schermata di login
- **WishCard**: pulsante "Rimuovi" ora in colore neutro per distinguerlo visivamente da "Modifica"
