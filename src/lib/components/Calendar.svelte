...
<div class="wrap">
  <button class="date-btn" onclick={(e) => { e.stopPropagation(); open = !open; }}>
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#C4622D" stroke-width="2.2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
...
<style>
  ...
  .date-btn {
    ...
    padding: 10px 11px; border-radius: 10px; border: 1.5px solid #D4A574;
    background: #FAF3E8; cursor: pointer; box-sizing: border-box; width: 100%;
  }
  .date-btn span { font-size: 11px; font-weight: 600; color: #3D2010; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .dropdown {
    position: absolute; top: 100%; left: 0; right: 0; z-index: 50;
    background: #FAF3E8; border: 1.5px solid #D4A574; border-radius: 14px;
    padding: 12px; margin-top: 4px;
  }
  .cal-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
  .cal-title { font-size: 12px; font-weight: 700; color: #3D2010; }
  .nav { all: unset; width: 28px; height: 28px; border-radius: 8px; background: #C4622D; color: #FFF; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-weight: 700; }
  .cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; }
  .day-name { text-align: center; font-size: 10px; font-weight: 700; color: #A07850; padding: 2px 0; }
  .day { all: unset; text-align: center; padding: 5px 2px; border-radius: 7px; font-size: 12px; cursor: pointer; color: #3D2010; display: block; width: 100%; }
  .day.today { background: #EDD9C0; color: #C4622D; font-weight: 700; }
  .day.selected { background: #C4622D; color: #FFF; font-weight: 700; }
</style>
