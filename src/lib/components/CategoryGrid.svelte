...
<div class="grid" style="grid-template-columns: repeat({columns},1fr)">
  {#each categories as cat}
    ...
    >
      <div style="color:{selected === cat.id ? cat.color : 'var(--text-muted)'}">{@html cat.svg}</div>
...
<style>
  ...
  .cat-btn {
    ...
    border: 2px solid var(--border-light); cursor: pointer; box-sizing: border-box; color: var(--text-muted);
  }
