...
export function listenRecurring(cb: (data: RecurringExpense[]) => void) {
  const rootRef = ref(db, ROOT + '/recurring');
  return onValue(rootRef, (snap) => cb(snap2arr<RecurringExpense>(snap.val())));
}
